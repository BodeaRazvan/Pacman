...
def depthFirstSearch(problem):
    ...
    while not stack.isEmpty():
        ...
        for succ, action, cost in problem.getSuccessors(top):
...
def breadthFirstSearch(problem):
    ...
    while not myQueue.isEmpty():
        ...
        if currentNode not in vistiedNodes:
            vistiedNodes.append(currentNode)
...
def uniformCostSearch(problem):
    ...
    vistiedNodes = []
    myQueue.push((startingNode, [], 0), 0)
    while not myQueue.isEmpty():
        currentNode, actions, cost = myQueue.pop()
        if currentNode not in vistiedNodes:
            vistiedNodes.append(currentNode)
            if problem.isGoalState(currentNode):
                return actions
            for nextNode, action, nextCost in problem.getSuccessors(currentNode):
                newAction = actions + [action]
                myQueue.push((nextNode, newAction,cost+nextCost),cost+nextCost)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not myPriorityQueue.isEmpty():
        (current, path, prevCost) = myPriorityQueue.pop()
        if current not in visited:
            visited.append(current)
            if problem.isGoalState(current):
                return path
            successors=problem.getSuccessors(current)
            for nextNode,nextMove,cost in successors:
                heuristicCost = cost + prevCost + heuristic(nextNode,problem)
                myPriorityQueue.push((nextNode,path+[nextMove],cost+prevCost),heuristicCost)
